...
output_path = r"G:\Shared drives\Ryoko and Hilary\SMSigxModel\analysis\2_data_input\LittleWashita"
# %%
...
file_names = ['ars_VW05.csv', 'ars_VW25.csv', 'ars_VW45.csv', 'ars_RAIN.csv', 'PET.csv', 'Q.csv']
...
df_to_save_forcing.to_csv(os.path.join(output_path, 'full', 'little_washita_2006_2012.csv'))

# %% 
area = 601 * 1E06
df_to_save_test = df_to_save[['RAIN', 'Q', 'sm']].copy()
df_to_save_test.rename(columns={'sm': 'Soil Moisture Content', 'RAIN': 'precip_rate', 'Q': 'Flow'}, inplace=True)
...
df_to_save_test['Total Discharge'] = df_to_save_test['Flow'] * area / 3600 # Is this correct? [m3/hr]? 
df_to_save_test.to_csv(os.path.join(output_path, 'full', 'test_sm_basinavg.csv'))
...
df_to_save_short_forcing.to_csv(os.path.join(output_path, 'short', 'little_washita_2006_2012.csv'))
df_to_save_short_test = df_to_save_test[start_date:end_date]
df_to_save_short_test.to_csv(os.path.join(output_path, 'short', 'test_sm_basinavg.csv'))
