...
def main(out_path="", config_path_CFE="", config_path_GLUE="", eval_criteria=dict()):
    ...
    out_path = (
        r"G:\Shared drives\Ryoko and Hilary\SMSigxModel\analysis\6_out\seasonsig_test"
    )
    i_run = 6
    ...
    sig_sim.movmean()
    season_trans_sim, start_dates_sim, end_dates_sim = sig_sim.calc_seasontrans(
        t_valley=t_valley
    )
    ...
    ax3.plot(
        sig_sim.tt.index,
        (sig_sim.tt.values - min(sig_sim.tt.values))
        / (max(sig_sim.tt.values) - min(sig_sim.tt.values)),
        alpha=1,
        label=sim_label,
        color=sim_color,
    )
    ...
    for i in range(len(start_dates_sim)):
        ax3.axvline(
            x=start_dates_sim[i], color=sim_color, label=None, alpha=0.5, linestyle="-"
        )
    for i in range(len(end_dates_sim)):
        ax3.axvline(
            x=end_dates_sim[i], color=sim_color, label=None, alpha=0.5, linestyle="--"
        )
    ax3.set_xlabel("Time")
