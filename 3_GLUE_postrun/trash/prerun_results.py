...
class GLUE(object):
    ...
    def post_evaluation(self, plot=True):
        ...
        try:
            df_pri_paras = pd.read_csv(
                os.path.join(self.path_GLUE_output, "parameter_priori.csv"), index_col=0
            )
        except:
            df_pri_paras = pd.read_csv(
                os.path.join(self.path_GLUE_output, "parameter_priori.xls"), index_col=0
            )

        try:
            df_eval_metrics = pd.read_csv(
                os.path.join(self.path_GLUE_output, "evaluations.csv"), index_col=0
            )
        except:
            df_eval_metrics = pd.read_csv(
                os.path.join(self.path_GLUE_output, "evaluations.xls"), index_col=0
            )

        try:
            df_eval_metrics_monthly = pd.read_csv(
                os.path.join(
                    self.path_GLUE_output, "post_evaluations_monthly_metrics.csv"
                ),
                index_col=0,
            )
        except:
            df_eval_metrics_monthly = pd.read_csv(
                os.path.join(
                    self.path_GLUE_output, "post_evaluations_monthly_metrics.xls"
                ),
                index_col=0,
            )
        ...
        for i in range(len(self.eval_criteria)):
            if self.eval_criteria[i]["metric"] == "season_transition":
                self.eval_names.append(f'{self.eval_criteria[i]["metric"]}_dry2wet_s')
                self.eval_names.append(f'{self.eval_criteria[i]["metric"]}_dry2wet_e')
                self.eval_names.append(f'{self.eval_criteria[i]["metric"]}_wet2dry_s')
                self.eval_names.append(f'{self.eval_criteria[i]["metric"]}_wet2dry_e')
                ...
                self.eval_names.append(
                    f'{self.eval_criteria[i]["metric"]} on {self.eval_criteria[i]["variable_to_analyze"]}'
                )
                self.eval_thresh.append(self.eval_criteria[i]["threshold"])

        for i in range(len(self.eval_names)):
            if ("KGE" in self.eval_names[i]) or ("NSE" in self.eval_names[i]):
                df_eval_metrics[self.eval_names[i] + "_Behavioral"] = (
                    df_eval_metrics[self.eval_names[i]] > self.eval_thresh[i]
                )
        ...
        df_eval_metrics["GLUE_results_Behavioral"] = df_eval_metrics[
            glue_judge_column
        ].all(axis="columns")

        # Generate random parameters
        self.behavioral_idx = df_eval_metrics[
            df_eval_metrics["GLUE_results_Behavioral"]
        ].index
        ...
        self.df_post_eval_mo = df_eval_metrics_monthly[
            df_eval_metrics_monthly["run_id"].isin(self.behavioral_idx.values)
        ].copy()
        ...
        self.df_post_eval.to_csv(
            os.path.join(self.out_path_per_senario, "post_evaluations.csv"),
            sep=",",
            header=True,
            index=True,
            encoding="utf-8",
        ...
        self.df_post_eval_mo.to_csv(
            os.path.join(
                self.out_path_per_senario, "post_evaluations_monthly_metrics.csv"
            ),
            sep=",",
            header=True,
            index=True,
        ...
        if plot:
            ...
            for i in range(nparas):
                ...
                df_pri_paras[target_para].plot.hist(
            ...
            f.savefig(
                os.path.join(self.out_path_per_senario, "param_dist.png"), dpi=600
            )
            del f
            print("------------- Saved figures -------------------")
            ...
            if hasattr(self, "df_post_paras"):
                for j in range(len(self.df_post_eval.columns)):
                    ...
                    f.savefig(
                        os.path.join(
                            self.out_path_per_senario,
                            "param_dotty_%s.png" % (target_eval),
                        ),
                        dpi=600,
                    )
            ...
            if hasattr(self, "df_post_paras"):
                ...
                for i in range(len(param_interest)):
                    for j in range(len(param_interest)):
                        ...
                        ax1 = f.add_subplot(
                            len(param_interest), len(param_interest), n_plot
                        )
                ...
                f.savefig(
                    os.path.join(
                        self.out_path_per_senario, "param_dotty_interaction.png"
                    ),
                    dpi=600,
                )
                del f
    ...
    def reproduce_behavioral_run(self, behavioral_param):
        ...
        for i, target_para in enumerate(sampled_params_set.index):
            if target_para in [
                ...
                self.cfe_cfg["soil_params"][target_para] = sampled_params_set[
                    target_para
                ]
            else:
    ...
    def calc_uncertainty_bounds(self, all_results, plot=True):
        ...
        for i in range(len(all_results)):
            if not hasattr(self, "df_behavioral_Q"):
                ...
                self.df_behavioral_Q = pd.concat(
                    [self.df_behavioral_Q, all_results[i][1]], axis=1
                )
                self.df_behavioral_SM = pd.concat(
                    [self.df_behavioral_SM, all_results[i][2]], axis=1
                )
        ...
        if hasattr(self, "df_Q_simrange"):
            self.df_Q_simrange.to_csv(
                os.path.join(self.out_path_per_senario, "quantiles_Q.csv"),
                sep=",",
                header=True,
                index=True,
                encoding="utf-8",
        ...
        if hasattr(self, "df_SM_simrange"):
            self.df_SM_simrange.to_csv(
                os.path.join(self.out_path_per_senario, "quantiles_SM.csv"),
                sep=",",
                header=True,
                index=True,
                encoding="utf-8",
