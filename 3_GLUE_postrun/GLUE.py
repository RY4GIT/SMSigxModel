...
class GLUE(object):
    ...
    def plot_uncertainty_bounds(self, observed_Q, observed_SM):
        ...
        """
        # Calculate weights
        weight = np.empty((len(self.posterior_eval_metrics), len(self.eval_names)))
        j = int(0)
        ...
        for i in range(len(self.eval_criteria)):
            if self.eval_criteria[i]['metric'] == "NSE" or self.eval_criteria[i]['metric'] == "KGE":
                ...
                weight[:, j] = ((self.posterior_eval_metrics[self.eval_names[j]] - self.eval_criteria[i]['threshold']) / sum(
                    self.posterior_eval_metrics[self.eval_names[j]] - self.eval_criteria[i]['threshold'])).to_numpy()
                j += int(1)
            elif self.eval_criteria[i]['metric'] == "season_transition":
                for k in range(4):
                    # For seasonal transition dates
                    weight[:, j + k] = triangle_weight(self.posterior_eval_metrics[self.eval_names[j + k]],
        ...
        for var_name in self.var_names:
            if var_name == "Flow":
                df_behavioral = self.behavioral_Q.copy()
                t_len = len(self.behavioral_Q)
            elif var_name == "Soil Moisture Content":
                df_behavioral = self.behavioral_SM.copy()
                t_len = len(self.behavioral_SM)
            np_behavioral = df_behavioral.to_numpy(copy=True)
