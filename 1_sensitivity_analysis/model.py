...
def duplicate_file(source_path):
    ...
    destination_path = os.path.join(directory, "temporary_config_for_sensitivity_analysis.json")
...
class CFEmodel():
    def __init__(self, config=None, problem=None, X=None):
        ...
        destination_path = duplicate_file(source_path = self.config['PATHS']['cfe_config'])
    ...
    def evaluate(self):
        ...
        sim = self.to_datetime(df=self.cfe_instance.cfe_output_data[["Time", self.eval_variable]], time_column="Time", format="%Y-%m-%d %H:%M:%S")
        obs = self.to_datetime(df=self.cfe_instance.unit_test_data[["Time", self.eval_variable]], time_column="Time", format=self.config['DATA']['evaldata_time_format'])

        if obs.index[0] != sim.index[0]:
            warnings.warn("The start of observation and simulation time is different by %s" % obs.index[0] - sim.index[0])

        if obs.index[-1] != sim.index[-1]:
            warnings.warn("The end of observation and simulation time is different by %s" % obs.index[-1] - sim.index[-1])
