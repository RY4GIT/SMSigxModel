...
class MyGLUE(object):
    ...
    def simulation(self):
                ...
                # Seasonal transition dates
                    ...
                    if n == 0:
                        sig_obs = SMSig(
                            ...
                            ts_value=obs_synced[
                                self.eval_criteia[i]["variable_to_analyze"]
                            ].to_numpy(),
                            plot_results=False,
                            plot_label="obs",
                        )
                        ...
                        season_trans_obs, _, _ = sig_obs.calc_seasontrans(
                            t_valley=t_valley
                        )
            ...
            if all(behavioral_flag):
                ...
                if flag_behavioral == 0:
                    ...
                    self.df_behavioral_Q = pd.concat(
                        [self.df_behavioral_Q, sim_synced["Flow"]], axis=1
                    )
        ...
        if n_behavioral != 0:
            ...
            self.df_post_paras = pd.DataFrame(
                param_values, index=self.post_rid, columns=param_names
            )
        ...
        if n_behavioral != 0:
            ...
            self.df_post_eval = pd.DataFrame(
                eval_values, index=self.post_rid, columns=self.eval_names
            )
    ...
    def post_process(self):
        ...
        for i in range(len(self.eval_criteia)):
            if (
                self.eval_criteia[i]["metric"] == "NSE"
                or self.eval_criteia[i]["metric"] == "KGE"
            ):
                # For Nash-Sutcliffe and Kling-Gupta Efficiency scores
                weight[:, j] = (
                    (
                        self.df_post_eval[self.eval_names[j]]
                        - self.eval_criteia[i]["threshold"]
                    )
                    / sum(
                        self.df_post_eval[self.eval_names[j]]
                        - self.eval_criteia[i]["threshold"]
                ...
                for k in range(4):
                    ...
                    weight[:, j + k] = triangle_weight(
                        ...
                        a=-1 * self.eval_criteia[i]["threshold"],
                        b=0,
    ...
    def to_csv(self):
        ...
        if hasattr(self, "df_post_paras"):
            self.df_post_paras.to_csv(
                os.path.join(self.out_path, "parameter_posterior.csv"),
                sep=",",
                header=True,
                index=True,
                encoding="utf-8",
        ...
        if plot_type == "timeseries":
            for var_name in self.var_names:
                ...
                df_simrange["lowerlim"].plot(
                    color="black", alpha=0.2, ax=ax2, label="_Hidden"
                )
