...
class GLUE(object):
    def __init__(self, config=None, criteria=None):
        ...
        # Quantile
        self.quantiles = [
            float(x.strip()) for x in self.config.get("GLUE", "quantiles").split(",")
        ]
        ...
        self.prior_params = pd.read_csv(
            os.path.join(GLUEprerun_output_path, "prior_parameters.csv"), index_col=0
        )
        self.prior_eval_metrics = pd.read_csv(
            os.path.join(GLUEprerun_output_path, "evaluation_metrics.csv"), index_col=0
        )
        self.prior_eval_metrics_monthly = pd.read_csv(
            os.path.join(GLUEprerun_output_path, "evaluation_metrics_monthly.csv"),
            index_col=0,
        )
    ...
    def apply_criteria(self):
        ...
        for _, criterion in self.criteria.full_criteria.items():
            self.prior_eval_metrics[
                criterion["metrics_fullname"] + "_Behavioral"
            ] = self.judge_behavioral(
        ...
        self.prior_eval_metrics["GLUE_results_Behavioral"] = self.prior_eval_metrics[
            glue_judge_column
        ].all(axis="columns")

        # Get the run_id of behavioral runs
        self.behavioral_run_ids = self.prior_eval_metrics[
            self.prior_eval_metrics["GLUE_results_Behavioral"]
        ].index
        ...
        self.posterior_eval_metrics_mo = self.prior_eval_metrics_monthly[
            self.prior_eval_metrics_monthly.index.isin(self.behavioral_run_ids.values)
        ].copy()

        # Save
        self.posterior_eval_metrics.to_csv(
            os.path.join(self.out_path, "post_evaluations.csv"), **self.kwargs_dict_csv
        )
        self.posterior_eval_metrics_mo.to_csv(
        ...
        behavioral_params = [
            [run_id, self.posterior_params.loc[run_id]]
            for run_id in self.behavioral_run_ids
        ]
    ...
    #########################
    # Uncertainty bounds calculation process
    #########################
    ...
    def get_uncertainty_bounds_for_all_variables(self):
        ...
        variable_map = {"Flow": "Q", "Soil Moisture Content": "SM"}
    ...
    def calc_uncertainty_bounds(self, var_attr, avg_weight):
        ...
        _dataseries_quantiles = np.array(
            [
                weighted_quantile(
                    values=row, quantiles=self.quantiles, sample_weight=avg_weight
                )
        ...
        dataseries_quantiles = pd.DataFrame(
            _dataseries_quantiles,
        ...
        if hasattr(self, f"quantile_{var_attr}"):
            dataseries_quantiles.to_csv(
                os.path.join(self.out_path, f"quantiles_{var_attr}.csv"),
                **self.kwargs_dict_csv,
            )
    ...
    def get_weights_for_multi_criteria(self):
        ...
        weights = np.empty(
            (len(self.behavioral_run_ids), len(self.criteria.full_criteria))
        )

        # Get weights for each criteria
        for i, (_, criterion) in enumerate(self.criteria.full_criteria.items()):
            weights[:, i] = self.calc_weights(
                self.posterior_eval_metrics[criterion["metrics_fullname"]],
                criterion["threshold"],
                criterion["metrics_fullname"],
            )
    ...
    def calc_weights(self, eval_metrics_values, threshold, metrics_fullname):
        if "SeasonTrans" in metrics_fullname:
            weight = triangle_weight(
                eval_metrics_values, a=-1 * threshold, b=0, c=threshold
            )
        else:
            weight = (
                (eval_metrics_values - threshold) / sum(eval_metrics_values - threshold)
            ).to_numpy()
        return weight
    ...
    def plot_parameter_distribution(self):
        ...
        for i, target_para in enumerate(self.prior_params.columns):
            ...
            # Plot histograms
            self.prior_params[target_para].hist(
                bins=10, alpha=0.4, ax=ax, color="#3182bd", label="Prior"
            )

            if hasattr(self, "posterior_params"):
                self.posterior_params[target_para].hist(
                    bins=10, alpha=0.8, ax=ax, color="#3182bd", label="Posterior"
                )
    ...
    def plot_parameter_dotty(self):
        ...
        eval_metrics = [
            col
            for col in self.posterior_eval_metrics.columns
            if "_Behavioral" not in col
        ]

        for target_eval in eval_metrics:
            f, axes = plt.subplots(
                nparas, 1, figsize=(4 * 4, 4 * nparas), constrained_layout=True
            )

            for i, target_para in enumerate(self.prior_params.columns):
                ax = axes[i]
                ax.scatter(
                    self.posterior_params[target_para],
                    self.posterior_eval_metrics[target_eval],
                    alpha=0.5,
                )
    ...
    def plot_parameter_interaction_dotty(self):
        ...
        f, axes = plt.subplots(
            n_params, n_params, figsize=(20, 20), constrained_layout=True
        )
        ...
        for i, para1 in enumerate(param_interest):
            for j, para0 in enumerate(param_interest):
                ax = axes[i, j]
                ax.scatter(
                    self.posterior_params[para0],
                    self.posterior_params[para1],
                    alpha=0.5,
                )
    ...
    def plot_uncertainty_bounds(self, observed_Q, observed_SM):
        ...
        for _, setting in settings.items():
            ...
            setting["df_simrange"]["lowerlim"].plot(
                color="black",
                alpha=0.2,
                ax=ax2,
                label=f"{self.quantiles[0]*100} percentile",
            )
            setting["df_simrange"]["upperlim"].plot(
                color="black",
                alpha=0.2,
                ax=ax2,
                label=f"{self.quantiles[2]*100} percentile",
            )
            setting["df_obs"].plot(
                color="black", alpha=1, ax=ax2, label=setting["obs_label"]
            )

            plt.fill_between(
                setting["df_simrange"].index,
                setting["df_simrange"]["upperlim"],
                setting["df_simrange"]["lowerlim"],
                facecolor="green",
                alpha=0.2,
                interpolate=True,
