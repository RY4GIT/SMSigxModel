...
def main(config_path):
    ...
    if glue_experiment.nrun > 1:
        ...
        all_results = pool.map(
            glue_experiment.reproduce_a_behavioral_run, behavioral_param_sets
        )
        pool.close()
        pool.join()
        ...
        all_results = [
            glue_experiment.reproduce_a_behavioral_run(
                behavioral_param_set=behavioral_param_sets[0]
            )
        ]
...
if __name__ == "__main__":
    main(config_path="4_GLUE_postrun/config.ini")
